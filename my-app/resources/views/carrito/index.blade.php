...
@section('content')
    <div class="section-header">
        ...
        <div class="section-muted">{{ $items->sum('cantidad') }} articulos</div>
    </div>

    @if($items->isEmpty())
        <p class="section-muted">Tu carrito esta vacio.</p>
        <a href="{{ route('productos.index') }}" class="btn-ghost mt-4">Ir a productos</a>
    @else
                ...
                <article class="product-card-container">
                    <div class="product-card-body">
                        ...
                        <div class="mt-3 flex items-center justify-between">
                            <span class="price-current">${{ number_format($item['precio'], 2) }}</span>
                            <span class="section-muted">Subtotal: ${{ number_format($item['precio'] * $item['cantidad'], 2) }}</span>
                        </div>
                    </div>
                </article>
            @endforeach
        ...
        <div class="mt-4">
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('productos.index') }}" class="btn-ghost">Seguir comprando</a>
